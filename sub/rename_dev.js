 ...
 *
 * 1.在SubStore内对节点重命名为：[n] emoji 国家代码 [a] ...
 * 2.过滤掉不规范命名的节点。例如，剩余,过期..
 * 
 * i=zh：机场节点名格式(en 国家代码 SG，enf 英文全称 Singapore，zh 中文全称 新加坡，enl 长国家代码 SGP)
 * n=xx：每个节点前面添加自义定机场名
...
var additionalOthers = $arguments.o || '{}';
    ...
    if (countries[item][1] === 1) {
      proxies.map((res) => {
        if (res.name.indexOf(countries[item][0]) !== -1) {
          res.name = res.name.replace(/1|0/g, '');
        };
      });
    };
  });
...
function operator(proxies) {
  ...
  proxies.map((res) => {
    ...
    Object.keys(others).forEach((elem, index) => {
      if (simplify(res.name).indexOf(elem) !== -1) {
        resultArray.splice(4, 0, others[elem]);
